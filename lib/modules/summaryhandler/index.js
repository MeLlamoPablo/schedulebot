	...
	static generateSummary(event) {
		return new Promise((fulfill, reject) => {
			...
			Promise.all([
				...
				summary += `**${status !== "expired" ? "Will attend" : "Did attend"}** (`
				...
				// If the event has expired, don't show the "type ... to confirm" information.
				// However, do show it if an inhouse is associated and the lobby isn't closed.
				if (
					status !== "expired"
					&&
					(inhouseProps !== null && lobbyStatus !== ELobbyStatus.CLOSED)
				) {
