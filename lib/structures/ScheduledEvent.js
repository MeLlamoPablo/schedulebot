...
class ScheduledEvent {
	...
	/**
	 * Determines whether or not the lobby for this event has ended.
	 *
	 * @return {Promise.<boolean>}
	 */
	getLobbyEnded() {
		return db.events.getLobbyEnded(this);
	}
	...
	/**
	 * Declares a event as "lobby ended". No lobby will be created again for that event.
	 *
	 * @return {Promise}
	 */
	setLobbyEnded() {
		return new Promise((fulfill, reject) => {
			db.events.updateLobbyEnded(this, true).then(fulfill).catch(reject);
		});
	}
